...
# Load the version from the appdef file.
$version = "";
try{
    [xml]$appDef = Get-Content "appdef.xml";
    $version = Select-Xml -Xml $appDef -XPath "//version"
}
catch{}

# Application details
$appFilePath = "bin\Debug\SoftAdvice.MFilesVAF.LoadingExample-" + $version + ".mfappx"
$appGuid = ""

# Target vault
$vaultName = "Sample Vault (JR)"

$config = Get-Content -Raw -Path "connection.json" | ConvertFrom-Json

$authType = $config.authType # 1 = MFAuthTypeLoggedOnWindowsUser
$userName = $config.username
$password = $config.password
$domain = ""
$spn = ""
$protocolSequence = "ncacn_http"
$networkAddress = "mfilesdev.cloudadvice.be"
$endpoint = 4466
